...
const Card = ({ label, value, sub }: { label: string; value: string; sub?: string }) => (
  <div className="rounded-lg p-4" style={{ background: "#ffffff", border: "1px solid #e5e7eb" }}>
    <div className="text-xs tracking-widest mb-1" style={{ color: "#6b7280", fontWeight: "500" }}>{label}</div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        {warehouses.map(wh => {
          const pct = wh.total > 0 ? (wh.stock / wh.total) * 100 : 0;
          ...
          return (
            <div key={wh.id} className="rounded-lg p-3" style={{ background: "#ffffff", border: "1px solid #e5e7eb" }}>
              <div className="flex justify-between text-xs mb-2" style={{ display: "flex", flexWrap: "wrap" }}>
                <span className="font-bold" style={{ color: "#00e5a0" }}>{wh.id}</span>
                <span className="font-mono">
                  {fmtQty(wh.stock)}/{fmtQty(wh.total)} kg
                </span>
              </div>
